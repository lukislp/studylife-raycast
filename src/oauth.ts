// Pure half of the login round trip: no Raycast APIs and no sockets in here, so the parts that
// are easy to get subtly wrong (PKCE shape, state comparison, callback parsing) are unit-testable.
//
// Ported verbatim in shape from studylife-vscode's oauth.ts, which is itself the generic-flow
// port of studylife-cli's login.py. Keep the wire shapes identical to that reference - the server
 ...
 * four are registered once for "studylife-raycast" via studylife-developers, and differ from
 * studylife-vscode's 8775-8778 (and studylife-cli's 8765-8768) so several clients can be logged
 * in at the same time.
 */
export const CANDIDATE_PORTS = [8795, 8796, 8797, 8798] as const;

export const DEFAULT_CLIENT_ID = "studylife-raycast";
