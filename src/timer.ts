...
// web app and Home Assistant already understand. Ported verbatim from studylife-vscode's timer.ts
// - the transition rules do not depend on the editor vs. launcher distinction at all.
//
 ...
 * Ported from studylife-vscode's timer.ts (pickTimerMode's model).
 */
 ...
 * stored in the user's settings and this extension has no scope to read them - so remaining time
 * is shown without a total rather than inventing one.
 */
 ...
 * (nothing running, or a custom mode whose length this extension cannot read).
 */
...
/** "24:13" - the shape a countdown wants, unlike formatHours' "24 h 13 min". */
