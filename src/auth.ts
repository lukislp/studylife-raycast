// The Raycast/socket half of the login: binds a loopback listener, sends the user to the
// browser, waits for the redirect, and exchanges the one-time assertion for this installation's
// API key. The wire shapes it speaks are in oauth.ts and mirror studylife-vscode's auth.ts (and,
// through that, studylife-cli's login.py).
//
// Raycast ships a convenience OAuth.PKCEClient wrapper, but it expects a standard `?code=...`
// redirect - StudyLife's flow returns `?assertion=...&state=...`, a real shape mismatch. So this
// ports vscode's raw loopback-HTTP-server approach directly instead: Raycast's Node runtime
// permits binding local ports via node:http exactly like any other Node process.
...
const API_KEY_STORAGE_KEY = "studylife.apiKey";

export class LoginError extends Error {}

/** The API key lives in Raycast's LocalStorage - the standard fallback extensions use for a
 *  manually-obtained token when not using the OAuth.PKCEClient wrapper. */
    ...
    const server: Server = createServer((req, res) => {
      ...
      res.end(
        "<!doctype html><meta charset=utf-8><title>StudyLife</title>" +
          ...
          "<h1>Connected</h1><p>You can close this tab and go back to Raycast.</p>",
      );
      settle?.(params);
    });
...
export async function runLogin(
  ...
  try {
    ...
    if (!statesMatch(result.state, state)) {
      throw new LoginError(
        "Rejected the login callback: its state did not match what this Raycast session sent. " +
          "Run Connect again.",
      );
    }
    if (!result.assertion) {
