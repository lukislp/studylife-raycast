...
describe("connect URL", () => {
  it("carries the four query parameters the server expects", () => {
    const url = new URL(
      buildConnectUrl("https://studylife.example.com", "studylife-raycast", "http://127.0.0.1:8795/callback", "st4te", "ch4llenge"),
    );
    expect(url.pathname).toBe("/connect/client/studylife-raycast");
    expect(url.searchParams.get("redirect_uri")).toBe("http://127.0.0.1:8795/callback");
  ...
  it("does not produce a double slash when the instance URL has a trailing one", () => {
    const url = buildConnectUrl("https://studylife.example.com/", "c", "http://127.0.0.1:8795/callback", "s", "c");
...
describe("loopback redirect", () => {
  ...
  it("uses ports that do not collide with studylife-vscode's 8775-8778 or studylife-cli's 8765-8768", () => {
    for (const port of CANDIDATE_PORTS) {
      expect(port).toBeGreaterThan(8778);
    }
  });
});
